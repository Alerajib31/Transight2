...
# Generate training data (optional, if historical_bus_data.csv doesn't exist)
python data_generator.py

# Train ML model (optional, if trained_model.json doesn't exist)
...
1. **Start Backend**: `python server.py` - Verify it loads the ML model and starts on port 8000
2. **Start Frontend**: `npm run dev` in frontend directory - Verify it compiles without errors
3. **Test API**: Visit `http://localhost:8000/docs` to test endpoints via Swagger UI
4. **Test Map Interface**: Open `http://localhost:5173` and verify:
   - Map loads centered on Bristol
   - Station markers are visible
   - Clicking a station shows approaching buses
...
The following API keys are hardcoded in `server.py` for demonstration purposes:
- BODS API Key: Present in `BODS_CREDENTIALS` dictionary
- TomTom API Key: Present in `TOMTOM_TRAFFIC_CONFIG` dictionary
...
2. **Traffic Data**: Real-time from TomTom API or time-based heuristics as fallback
...
If BODS API returns no data, the system automatically generates simulated buses for demonstration. This is controlled by `SIMULATED_BUSES_ENABLED = True` in `server.py`.
...
1. **YOLO model not found**: Ensure `yolo.pt` is in the `Bus Predictor` directory
2. **CV module can't find video**: Place videos in `Bus Predictor/videos/` directory
3. **Frontend can't connect to backend**: Verify backend is running on port 8000
4. **CORS errors**: Check CORS middleware configuration in `server.py`
